...
class ContentService:
    """
    ContentService class provides methods for fetching content from a URL using Jina AI Reader
    and analyzing it with OpenAI.

    __init__(self, jina_api_key: str, openai_api_key: str, categories: List[str]):
        Initialize ContentService with Jina API key, OpenAI API key, and a list of categories.
    ...
    def __init__(self, jina_api_key: str, openai_api_key: str, categories: List[str],
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(jina_url, headers=headers) as response:
                    ...
                    content_diff = self.markdown_storage.get_content_diff(url, new_content)
    ...
    async def analyze_content(self, content_result: Dict, prompt_template: str) -> List[Dict]:
        """
        Analyze content changes using OpenAI.

        :param content_result: Dictionary from fetch_content containing content and diff
        :param prompt_template: The template of the prompt to be used for generating the analysis.
        :return: A list of dictionaries containing the parsed analysis results from the response.
        """
        log.info(f"Analyzing {content_result['url']}")


        if not content_result["diff"].has_changes:
            log.info(f"No changes detected for {content_result['url']}, using stored items")
        ...
        new_items = self.item_storage.filter_new_items(content_result['url'], parsed_items)
    ...
    def _extract_title(markdown: str) -> str:
        """
        :param markdown: A string representing the markdown content from which to extract the title.
        :return: The extracted title as a string if found, otherwise "No title found".
        """
    ...
    def _parse_markdown_items(self, markdown_text: str) -> List[Dict]:
        ...
        :return: A list of dictionaries, each representing a parsed markdown item with keys like title, source, link, and categories.
        """
        items = []
        current_item = None
