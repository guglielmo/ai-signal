...
from aisignal.core.interfaces import OperationResult, UserContext
...
def alternative_user_context():
    ...
    return UserContext(
        ...
        session_id="alternative_session",
        preferences={
...
@pytest.fixture
async def core_service(container):
...
@pytest.fixture
async def storage_service(container):
...
@pytest.fixture
async def config_manager(container):
...
@pytest.fixture
async def content_service(container):
...
def test_helpers():
    ...
    class TestHelpers:
        ...
        def assert_operation_success(result: OperationResult, message: str = ""):
            ...
            assert result.success, f"Operation failed: {result.error}. {message}"
        ...
        def assert_operation_failure(
            ...
            assert not result.success, "Expected operation to fail"
            if expected_error:
                assert (
